...
static HAL_StatusTypeDef bme280_write8(BME280_Handle_t *dev, uint8_t reg, uint8_t value)
{
  return HAL_I2C_Mem_Write(dev->i2c, dev->address, reg, I2C_MEMADD_SIZE_8BIT, &value, 1U, HAL_MAX_DELAY);
}

static HAL_StatusTypeDef bme280_read8(BME280_Handle_t *dev, uint8_t reg, uint8_t *value)
{
  return HAL_I2C_Mem_Read(dev->i2c, dev->address, reg, I2C_MEMADD_SIZE_8BIT, value, 1U, HAL_MAX_DELAY);
}

static HAL_StatusTypeDef bme280_read_block(BME280_Handle_t *dev, uint8_t reg, uint8_t *buf, uint16_t len)
{
  return HAL_I2C_Mem_Read(dev->i2c, dev->address, reg, I2C_MEMADD_SIZE_8BIT, buf, len, HAL_MAX_DELAY);
}
